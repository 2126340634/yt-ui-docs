...
```vue
<yt-collapse :titleList="['左侧图标面板1', '左侧图标面板2']" icon-side="left" @change="handleCollapseChange">
  <template v-slot:collapse-item-0>
...
```vue
<yt-collapse :titleList="['个人信息', '工作经历', '教育背景', '技能证书']" icon-side="left" :gap="10" @change="handleResumeChange">
  <template v-slot:collapse-item-0>
