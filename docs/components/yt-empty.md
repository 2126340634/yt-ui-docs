...
```vue
<template>
  <view v-if="isError" class="error-state">
    <yt-empty type="fail" :size="60" />
...
```vue
<template>
  <view v-if="isComplete" class="complete-state">
    <yt-empty type="done" :size="72" />
...
```vue
<template>
  <view v-if="isWinner" class="winner-state">
    <yt-empty type="champion" :size="100" />
...
<template>
  <view v-if="state !== 'loading'" class="state-container">
    <yt-empty :type="emptyType" :size="emptySize" />
...
<template>
  <view class="data-container">
    ...
    <view v-else-if="dataState === 'empty'" class="state-view">
      <yt-empty type="empty" :size="emptySize" />
      <text>暂无数据</text>
      <yt-button @click="loadData">重新加载</yt-button>
    </view>
    <view v-else-if="dataState === 'success'" class="state-view">
      <yt-empty type="done" :size="emptySize" />
      <text>数据加载成功！</text>
    </view>
    <view v-else-if="dataState === 'error'" class="state-view">
      <yt-empty type="error" :size="emptySize" />
      <text>数据加载失败</text>
