...
```vue
<yt-tabbar centerIcon="Plus" :list="[{ icon: 'Home' }, { icon: 'Search' }, { icon: 'Bell' }, { icon: 'User' }]" />
```
...
```vue
<yt-tabbar v-model="currentTab" :list="tabList" @change="handleTabChange" @centerClick="handleCenterClick" />
```
