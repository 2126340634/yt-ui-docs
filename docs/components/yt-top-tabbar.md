...
```vue
<yt-top-tabbar v-model="currentTab" :list="['全部', '进行中', '已完成']" @change="handleTabChange" />
```

### 完整示例

```vue
<yt-top-tabbar v-model="activeTab" :list="['首页', '发现', '消息', '我的']" :fixed="true" :zIndex="999" @change="handleTabSwitch" />
```
