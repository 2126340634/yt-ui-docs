...
function createMcpServer(): McpServer {
  ...
  server.tool(
    "list_components",
    "列出所有可用的 Yt-UI 组件名称和简介",
    {},
    async () => {
      const lines = components.map(
        (c) =>
          `- **${c.name.split(" ")[0]}**：${c.description || c.props[0]?.description || "暂无描述"}`
      );
      return {
        content: [
          {
  ...
  server.tool(
    ...
    async ({ query }) => {
      ...
      for (const c of components) {
        ...
        const matchedProps = c.props.filter(
          (p) =>
        ...
        if (matchedProps.length > 0) {
          ...
          if (!reason)
            reason = `Props 匹配：${matchedProps.map((p) => p.name).join("、")}`;
        }

        const matchedEvents = c.events.filter(
          (e) =>
        ...
        if (matchedEvents.length > 0) {
          ...
          if (!reason)
            reason = `Events 匹配：${matchedEvents.map((e) => e.name).join("、")}`;
        }
